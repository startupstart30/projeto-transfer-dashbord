...
import { Loader2, Calendar, Clock, User, Car, MapPin, Briefcase, Plus, X } from "lucide-react";
    ...
    if (form.extras.length > 0) {
      ...
      if (extrasData.length > 0) {
        const { error: extrasError } = await supabase
          ...
          .insert(extrasData);
        if (extrasError) {
          setError("Reserva salva, mas erro ao salvar extras.");
          setLoading(false);
          return;
        }
      }
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-6 flex flex-col gap-6">
        ...
        {success && <div className="text-green-600 text-sm font-semibold">Reserva cadastrada com sucesso!</div>}
        <div className="flex gap-2 justify-end">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading && <Loader2 className="animate-spin w-4 h-4" />} Salvar Reserva
          </button>
        </div>
      </form>
    </div>
